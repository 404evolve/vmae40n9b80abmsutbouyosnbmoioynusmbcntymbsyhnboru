...
def initialize_database():
    ...
    try:
        cursor.execute("ALTER TABLE users ADD COLUMN checked_captcha INTEGER DEFAULT 0")
        print('Поле checked_captcha добавлено в таблицу "users"')
    except sqlite3.OperationalError as e:
        if "duplicate column name" in str(e):
            print('Выполнено подключение к полю checked_captcha в таблице "users".')
        else:
            print(f"Ошибка при добавлении поля checked_captcha: {e}")
    ...
    if cursor.execute('SELECT name FROM sqlite_master WHERE type="table" AND name="flyer_task"').fetchone() is None:
        cursor.execute("""
...
def update_checked_captcha(user_id: int):
    with sqlite3.connect(DATABASE_NAME) as conn:
        cursor = conn.cursor()
        cursor.execute("UPDATE users SET checked_captcha = 1 WHERE id = ?", (user_id,))
        conn.commit()

def get_checked_captcha(user_id: int) -> bool:
    with sqlite3.connect(DATABASE_NAME) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT checked_captcha FROM users WHERE id = ?", (user_id,))
...
def get_all_links_opnotcheck() -> list[str]:
    try:
        with sqlite3.connect(DATABASE_NAME) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT link FROM op_not_checker")
        ...
        with sqlite3.connect(DATABASE_NAME) as conn:
            ...
            cur.execute("""
                ...
                WHERE referral_id = ?
                  AND registration_time BETWEEN ? AND ?
            """, (user_id, start_ts, end_ts))
            (my_cnt,) = cur.fetchone() or (0,)

            if my_cnt == 0:
                return f"<b>🚫 Ты не попал в топ!</b> | <code>0</code> рефералов."
            
            cur.execute("""
                SELECT COUNT(DISTINCT cnt) + 1
                FROM (
                  SELECT COUNT(*) AS cnt
                  FROM users
                ...
                WHERE cnt > ?
            """, (start_ts, end_ts, my_cnt))
            (rank_value,) = cur.fetchone() or (1,)

    except Exception as e:
        return f"Ошибка при получении вашего места в топе: {e}"

    return f"<b>🏅 Ты на {rank_value} месте</b> | <code>{my_cnt}</code> рефералов."
