...
from aiogram.types import Message, CallbackQuery, InlineKeyboardMarkup, InlineKeyboardButton, InputFile, LabeledPrice, PreCheckoutQuery, BufferedInputFile, WebAppInfo
...
async def request_op(user_id, chat_id, first_name, language_code, bot: Bot, ref_id=None, gender=None, is_premium=None):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post('https://api.subgram.ru/request-op-tokenless/', headers=headers, json=data, ssl=False) as response:
            ...
            elif response_json.get('status') == 'register':
                reg_url = response_json.get('additional').get('registration_url')
                if ref_id:
                    await show_register(chat_id, bot, ref_id=ref_id, reg_url=reg_url)
                else:
                    await show_register(chat_id, bot, reg_url=reg_url)
            return response_json.get("status")

async def show_register(chat_id, bot: Bot, ref_id=None, reg_url=None):
    user_id = chat_id
    ...
    needed_calldata = f"subgram-op:{ref_id or 'None'}"
    builder.button(
        text="✅ Продолжить ",
        callback_data=needed_calldata
    )

    builder.adjust(1)

    # await bot.send_message(user_id, "<b>⭐️ Для получения звёздочек</b>\n\n<i>Ответь всего на пару вопросов 👇 (Шаг 1 из 2)</i>", parse_mode='HTML', reply_markup=builder.as_markup())
    await bot.send_photo(user_id, photo=FSInputFile('photos/register.png'), caption="<b>⭐️ Для получения звёздочек</b>\n\n<i>Ответь всего на пару вопросов 👇 (Шаг 1 из 2)</i>", parse_mode='HTML', reply_markup=builder.as_markup())
...
async def request_task(user_id, chat_id, first_name, language_code, bot: Bot):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post('https://api.subgram.ru/request-op/', headers=headers, json=data, ssl=False) as response:
            if not response.ok or response.status != 200:
...
async def check_task(user_id, links):
    ...
    # print(f"[check task] send data: {data}")
    async with aiohttp.ClientSession() as session:
        async with session.post('https://api.subgram.ru/get-user-subscriptions', headers=headers, json=data) as response:
            ...
            # print(f"[check task] response: {response_json}")

            if response_json.get('code') != 200:
                logging.error('SubGram вернул ошибку: %s' % response_json.get('message'))
                return {}

            sponsors = (response_json.get('additional') or {}).get('sponsors') or []
            returned = {s.get('link'): s.get('status') for s in sponsors if s.get('link')}

            result = {}
            for link in links:
                result[link] = returned.get(link)
            return result
...
async def adminpanel_command(message: Message, bot: Bot):
    if message.from_user.id in admins_id:
        ...
        markup_admin = builder_admin.adjust(1, 3, 3, 2, 2, 1, 3, 3, 1).as_markup()
...
async def click_star_callback(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if chat_id not in admins_id:
        ...
        if required_subscription and not await check_subscription(user_id, required_subscription, bot):
            return
...
async def subgram_task_callback(call: CallbackQuery, bot: Bot):
    try:
        ...
        # print(f"[subgram-task] Links: {links}")
        response = await check_task(user_id=user_id, links=links)
        # print(f"[subgram-task] Response: {response}")
        if not all(status == 'subscribed' for status in response.values()):
            await bot.answer_callback_query(call.id, "❌ Вы всё ещё не подписаны на все каналы!", show_alert=True)
            await bot.delete_message(chat_id=call.from_user.id, message_id=call.message.message_id)
            await show_task(call.from_user.id, links, bot)
            return
...
async def subgram_op_callback(call: CallbackQuery, bot: Bot):
    try:
        ...
        if response == 'register':
            await bot.answer_callback_query(call.id, "❌ Вы не прошли регистрацию!", show_alert=True)
            try:
                await bot.delete_message(chat_id=call.from_user.id, message_id=call.message.message_id)
            except:
                pass
...
async def send_main_menu(user_id: int, bot: Bot, ref_id: Optional[int] = None):
    try:
        ...
        if user_exists(user_id):
            ...
            if user_id not in admins_id:
                response = await request_op(
                        ...
                        language_code=None,
                        bot=bot,
                        ref_id=None if ref_id is None else ref_id,
                        is_premium=getattr(user, 'is_premium', None)
                    )
                ...
                if required_subscription and not await check_subscription(user_id, required_subscription, bot, ref_id if ref_id is not None else None):
                    return
        else:
            if required_subscription and not await check_subscription(user_id, required_subscription, bot, ref_id if ref_id is not None else None):
                return
            if ref_id is not None:
                await handle_referral_bonus(ref_id, user_id, bot)
            add_user(user_id, user.username, None if ref_id is None else ref_id)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=payload, ssl=False) as response:
                ...
                print("Data from flyer task: ", data)
...
async def tasks_callback(call: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        if tasks == 'ok':
            ...
            print("Getted tasks flyer: ", tasks_list)
            if not tasks_list:
            ...
            for task in tasks_list:
                ...
                # print(f"[flyer-task] Signature: {signature}, UserID: {user_id}, Hash: {task_hash}")
                # print(f"[flyer-task] task_hash: {task_hash}, completed: {is_flyer_task_completed(task_hash)}")
